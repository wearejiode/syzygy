...
3. **Follow the Meta-Process**:
   ...
   - **Repeat**:  Increment the `Revision` number after each major change and update the `updated` date.
...
6. **Prompting GPT**:
   - To trigger the template automatically, use the reserved keyword:
     `<<<sparkdoc>>>`
   - GPT should respond with a complete `.md` file using the format above and generate all missing content (title, tags, description, overview, and any additional sections. Gpt should also assign or update frontmatter updated date and increment the revision by +1).
